...
function bindSystemPromptDebugUIOnce() {
    if (window.__systemPromptDebugBound) return;
    window.__systemPromptDebugBound = true;

    const container = document.getElementById('system-prompt-debug');
    if (!container) return;

    const toggleBtn = document.getElementById('system-prompt-debug-toggle');
    const copyBtn = document.getElementById('system-prompt-debug-copy');
    const contentEl = document.getElementById('system-prompt-debug-content');

    const readExpanded = function () {
        try {
            return localStorage.getItem('system_prompt_debug_expanded') === 'true';
        } catch (e) {
            return false;
        }
    };

    const writeExpanded = function (v) {
        try {
            localStorage.setItem('system_prompt_debug_expanded', v ? 'true' : 'false');
        } catch (e) { }
    };

    const applyExpanded = function (expanded) {
        const isOpen = !!expanded;
        window.__systemPromptDebugExpanded = isOpen;
        if (contentEl) contentEl.style.display = isOpen ? 'block' : 'none';
        if (toggleBtn) toggleBtn.textContent = isOpen ? '收起' : '展开';
        writeExpanded(isOpen);
    };

    applyExpanded(readExpanded());

    if (toggleBtn) {
        toggleBtn.addEventListener('click', function () {
            applyExpanded(!window.__systemPromptDebugExpanded);
        });
    }
    ...
    if (copyBtn) {
        copyBtn.addEventListener('click', async function () {
            ...
            const payload = buildMergedSystemPromptForDebug(rid, '') || '';
            if (!payload) return;
...
function updateSystemPromptDebugUIImpl(roleId, prompt) {
    ...
    if (!isShowSystemPromptInChatEnabled()) {
    ...
    const merged = buildMergedSystemPromptForDebug(rid, typeof prompt === 'string' ? prompt : '');

    if (!merged) {
        contentEl.textContent = '（本次尚未生成系统提示。点一次爱心/触发 AI 后会显示本次请求实际发送的 system prompt。）';
        return;
    }

    contentEl.textContent = merged;
}
...
if (!window.__systemPromptDebugListenerBound) {
    ...
    try {
        window.addEventListener('ai:systemPromptUpdated', function (e) {
            ...
            if (!rid || rid === String(window.currentChatRole || '')) {
