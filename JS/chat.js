    ...
    if (chatRoom || historyBox) {
        ...
        if (startIndex > 0) {
            ...
            loadMoreBtn.onclick = function () {
                // 点击后简单粗暴：重新渲染全部（或者你可以写更复杂的逻辑加载前50条）
                // 这里为了代码简单，点击后渲染全部（可能会卡一下，但用户有心理准备）
                loadMoreBtn.innerText = "加载中...";
                setTimeout(() => {
                    historyBox.innerHTML = ""; // 清空
                    window.lastRenderedTime = 0; // 重置时间戳
                    historyData.forEach(msg => appendMessageToDOM(msg)); // 渲染全部
                }, 50);
            };
...
function getRoleApiMemoryCfg(roleId) {
    ...
    if (!id) return { limitOverride: null, limitOverrideUntil: 0, clearedAt: 0, redactions: [] };
    const all = loadApiMemoryCfgAll();
    const cfg = all[id] && typeof all[id] === 'object' ? all[id] : {};
    const next = {};
    next.limitOverride = cfg.limitOverride != null ? parseInt(cfg.limitOverride, 10) : null;
    if (!isFinite(next.limitOverride) || next.limitOverride <= 0) next.limitOverride = null;
    next.limitOverrideUntil = cfg.limitOverrideUntil != null ? Number(cfg.limitOverrideUntil) : 0;
    if (!isFinite(next.limitOverrideUntil) || next.limitOverrideUntil < 0) next.limitOverrideUntil = 0;
...
function buildApiMemoryHistory(roleId, baseHistory) {
    ...
    const now = Date.now();
    if (cfg.limitOverride != null) {
        if (cfg.limitOverrideUntil && now > cfg.limitOverrideUntil) {
            cfg.limitOverride = null;
            cfg.limitOverrideUntil = 0;
            saveRoleApiMemoryCfg(roleId, cfg);
        } else if (!cfg.limitOverrideUntil) {
            cfg.limitOverrideUntil = now + 15 * 60 * 1000;
            saveRoleApiMemoryCfg(roleId, cfg);
        }
    }
    const clearedAt = normalizeTs(cfg.clearedAt);
    const redactions = Array.isArray(cfg.redactions) ? cfg.redactions.map(normalizeRedaction).filter(Boolean) : [];
    const history = Array.isArray(baseHistory) ? baseHistory.slice() : [];
    let filtered = history.filter(m => {
        if (!m) return false;
        const ts = normalizeTs(m.timestamp);
        if (clearedAt && ts && ts <= clearedAt) return false;
    ...
    filtered.sort(function (a, b) {
        return normalizeTs(a && a.timestamp) - normalizeTs(b && b.timestamp);
    });
    const limitSetting = parseInt(localStorage.getItem('chat_memory_limit')) || 50;
    const limit = cfg.limitOverride != null ? cfg.limitOverride : limitSetting;
    const nonSystem = filtered.filter(m => m && (m.role === 'me' || m.role === 'ai'));
    const tail = nonSystem.slice(-Math.max(0, limit | 0));
    const tailMinTs = tail.length ? normalizeTs(tail[0].timestamp) : Infinity;
    const systemTail = filtered.filter(m => m && m.role === 'system' && normalizeTs(m.timestamp) >= tailMinTs);
    const merged = tail.concat(systemTail);
...
window.compressApiMemoryTokens = function (roleId) {
    ...
    const cfg = getRoleApiMemoryCfg(id);
    cfg.limitOverride = 20;
    cfg.limitOverrideUntil = Date.now() + 15 * 60 * 1000;
    saveRoleApiMemoryCfg(id, cfg);

    // 增加成功提示
    alert("✅ 记忆压缩成功！\n\nAI 的后台 Tokens 已大幅释放。\n现在发给 AI 的上下文仅包含：\n1. 核心人设与世界书\n2. 长期记忆档案\n3. 最近的 20 条对话。\n\n此压缩状态将自动在约 15 分钟后恢复默认。\n（注意：你屏幕上的聊天记录不会被删除）");
};
...
window.clearRoleApiMemory = function (roleId) {
    ...
    // 增加二次确认和成功提示
    if (!confirm("⚠️ 危险操作确认：\n\n确定要彻底清除角色的记忆上下文吗？\n清除后，AI 将完全忘记之前的对话，像初次见面一样重新认识你。\n\n（你的屏幕上依然会保留聊天记录 UI）")) {
        return;
    }

    const cfg = getRoleApiMemoryCfg(id);
    cfg.clearedAt = Date.now(); // 记录清除时间，这时间之前的消息都不会发给 API
    cfg.limitOverride = null;
    cfg.redactions = [];
    saveRoleApiMemoryCfg(id, cfg);

    alert("♻️ 记忆已彻底清除！\nAI 现在处于出厂状态，快去和 Ta 重新打个招呼吧！");
};
            ...
            function type() {
                if (index < fullText.length) {
                    ...
                    if (fullText[index] === '<' && fullText.indexOf('>', index) !== -1) {
                        ...
                        textEl.innerHTML += fullText.substring(index, endTagIndex + 1);
                        index = endTagIndex + 1;
                    } else {
...
(function () {
    ...
    function buildWorldBookPrompt(worldbookId) {
        const wbId = String(worldbookId || '').trim();
        if (!wbId) return '';
        if (!window.worldBooks) return '';

        let wbPrompt = '';
        if (wbId.startsWith('CATEGORY:')) {
            const targetCat = wbId.split(':')[1];
            const allIds = Object.keys(window.worldBooks || {});
            const catBooks = allIds
                .map(id => window.worldBooks[id])
                .filter(b => {
 ...
 - 严禁桥接句式（"非但...而且...", "那不是...而是...", "好像是..."）。`; 
    ...
    function ensureWorldBookOptions() {
        const select = document.getElementById('offline-worldbook-select');
        if (!select) return;
        if (typeof window.getWorldBookOptionsHTML === 'function') {
            select.innerHTML = window.getWorldBookOptionsHTML('');
        } else {
            select.innerHTML = `<option value="">-- 不关联 (默认) --</option>`;
        }
        select.value = '';
    }
    ...
    function startNewOfflineSession() {
        ...
        const wbSelect = document.getElementById('offline-worldbook-select');
        const worldbookId = wbSelect ? String(wbSelect.value || '').trim() : DEFAULT_WB_ID;
        ...
        const session = {
            id: 'offline_' + now,
            roleId,
            worldbookId: worldbookId || DEFAULT_WB_ID,
