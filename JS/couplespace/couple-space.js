/* ==========================================================================
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            if (link.hasCoupleLinked && link.roleId) {
                state.missCountRole = loadAiMissCountForToday(link.roleId);
                updateStatsUI();
            }
        ...
        async function maybeGenerateRoleDiariesOnEnter() {
            ...
            const clean = cleanChatHistoryForMemory(history);
            const historyForApi = buildHistoryForApi(roleId, clean);
            const shortTermMemory = buildShortTermMemoryText(historyForApi, parseInt(localStorage.getItem('chat_memory_limit')) || 30);
        ...
        function updateStatsUI() {
            document.getElementById('miss-count-user').innerText = state.missCountUser;
            document.getElementById('miss-count-role').innerText = state.missCountRole;
        }
        ...
        function renderMoodPage() {
            ...
            calendarEl.innerHTML = `
                <div class="mood-calendar-header">
                    ...
                    <div class="mood-calendar-meta">${metaText}</div>
                </div>
                ${bodyHtml}
            `;
        ...
        function scheduleCharacterReplyForUserDiary(dateKey) {
            const key = String(dateKey || '').trim();
            if (!key) return;
            const link = getCoupleLinkState();
            if (!link || !link.hasCoupleLinked || !link.roleId) return;
