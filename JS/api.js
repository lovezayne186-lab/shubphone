...
try {
    ...
    // 2. 处理表情包 (保持原样)
    if (type === 'sticker') {
        if (isMe) return '[用户发送了一个表情包]';
        if (isAi) return '[你发送了一个表情包]';
        return '[发送了一个表情包]';
    }
    ...
    function buildApiMessages(finalSystemPrompt, historyMessages, userMessage) {
        ...
        if (imageUrls.length > 0) {
            ...
            messages.push({
                role: "system",
                content: `[系统通知：用户刚才发送了${imageCount}张真实照片（不是表情包）。你可以直接看到这些照片的画面内容，请根据你看到的画面进行回复。若用户明确要求你换头像且你同意，你必须设置 actions.changeAvatar = { "index": N }。注意：如果有多张图，尤其是情头或成对头像，你必须选出真正代表“你自己”的那张，不能默认第一张；如果无法判断，就不要触发 changeAvatar，并自然追问用户。]`
            });
        }
    ...
    try {
        ...
        async function doRequest(messagesToSend) {
            const controller = !softTimeoutNoAbort && typeof AbortController !== 'undefined' ? new AbortController() : null;
...
- events：写“聊天记忆/关系进展”（不只限重大事件）；每条必须以具体日期开头，例如“1月28日：”或“1月28日-1月30日：”，且能在对话中找到依据。
