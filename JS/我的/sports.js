var SportsLeaderboard = (function () {
    ...
    var storeUnsub = null;
    var PENDING_SYSTEM_CONTEXT_KEY = 'wechat_pending_system_context_v1';
    ...
    function readPendingSystemContext() {
        try {
            var raw = localStorage.getItem(PENDING_SYSTEM_CONTEXT_KEY);
            if (!raw) return null;
    ...
    function buildUserStepsSystemPrompt(steps) {
        return [
            '[系统内部提示：用户刚刚在运动后台更新了ta今天的真实步数为 ' + String(steps) + ' 步。请在你的下一次回复中，自然地提及这件事情。',
            '互动规则：',
            '1. 结合你当前的【角色人设】以及你刚才生成的【你的步数/运动轨迹】。',
            '2. 如果 ' + String(steps) + ' < 2000 步：判断用户今天很宅、太累或者生病了。根据人设进行吐槽（如傲娇）、关心或催促运动。',
            '3. 如果 2000 <= ' + String(steps) + ' <= 10000 步：属于正常通勤或散步，可以像日常聊天一样询问去了哪里。',
            '4. 如果 ' + String(steps) + ' > 15000 步：判断用户今天走了非常多路（如旅游、逛街、剧烈运动）。必须表现出惊讶，并根据人设表达心疼或赞叹。',
            '5. 重要：必须保持人设！语气要自然，可以进行步数对比（例如：“我都走了xx步，你才xx步”），绝对不要像机器人一样生硬地播报数字。]'
        ].join('\n');
    }
    ...
    function enqueueUserStepsSystemContext(steps) {
        ...
        n = Math.floor(n);
        var pending = readPendingSystemContext();
        if (pending && pending.content && pending.content.indexOf('真实步数为 ' + String(n) + ' 步') !== -1) return false;
        var payload = {
            id: Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 10),
            content: buildUserStepsSystemPrompt(n),
            createdAt: Date.now(),
            type: 'user_steps'
        };
        try {
            localStorage.setItem(PENDING_SYSTEM_CONTEXT_KEY, JSON.stringify(payload));
            return true;
        } catch (e) {
            return false;
        }
