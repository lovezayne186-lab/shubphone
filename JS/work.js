(function () {
    ...
    async function requestAIWorkReport(roleId, assignedJobName) {
        ...
        const systemPrompt = buildWorkReportSystemPrompt(characterSettings, worldbook, jobHistory, assignedJob);
        ...
        for (let attempt = 0; attempt < attemptMax; attempt++) {
            const userContent = attempt === 0
                ? '请严格输出 JSON，不要包含任何多余文本。'
                : '请重新生成：job_name 必须是兼职/打工/零工，且不能与最近10条历史兼职重复。只输出 JSON。';
    ...
    function fallbackWorkReport(roleId, assignedJobName, currentJob) {
        ...
        const diaryTpl = {
            calm: `今天做了${jobName}，把该收尾的都收好。路过窗边时风有点凉，我停了一下，心里反而安静下来。`,
            gentle: `今天忙着${jobName}，手一直没停。收工时闻到一点淡淡的味道，突然觉得这一整天也算温柔。`,
            lively: `今天的${jobName}节奏很快！我一边忙一边想办法把事情做顺，结果遇到个小插曲，居然也挺好笑的。`,
            tsundere: `今天做了${jobName}，也就那样吧。只是收尾的时候发生了点小事，我……没放在心上。`,
            snarky: `今天的${jobName}不算轻松，但还在可控范围。碰到点小状况，处理完就行。`
        };
        return {
            job_name: jobName,
            job_fit: '一般',
            diary: diaryTpl[tone] || diaryTpl.calm,
            gift: {
                name: jobName + '的小纪念',
                icon: (currentJob && currentJob.emoji) ? String(currentJob.emoji) : '🎁',
                description: '今天顺手留下一点小东西，带给你。'
            }
        };
    }
    ...
    function beginGenerateWorkReport(roleId, assignedJobName) {
        ...
        if (prevReq && prevReq.jobName === jobName && prevReq.promise) return prevReq.promise;
        ...
        runtime.workReportReqByRole[roleId] = { jobName: jobName, promise: p };
        return p;
    }
