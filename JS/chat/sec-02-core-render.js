...
function getChatViewportKeyboardOffset() {
    try {
        ...
        if (!vv) return 0;
        const raw = Math.round(window.innerHeight - vv.height - (vv.offsetTop || 0));
...
function bindChatViewportLayoutSyncOnce() {
    ...
    const syncLayout = function () {
        ...
        const keyboardOffset = isEditableElementActive() ? getChatViewportKeyboardOffset() : 0;

        // 只在“确实弹起键盘”时才用 visualViewport 强行改高度。
        // 否则会把容器缩成可视区域高度，导致底部露出一条“系统条/空白”。
        if (chatView) {
            if (keyboardOffset > 0 && window.visualViewport) {
        ...
        if (historyBox) {
            if (keyboardOffset > 0) {
                // 取消写死的底部 padding，因为已经缩小了外层容器高度
                historyBox.style.scrollPaddingBottom = '0px';
            } else {
