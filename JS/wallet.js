...
const Wallet = (function() {
    ...
    function renderTransactions() {
        ...
        filtered.forEach(t => {
            ...
            const peer = (t && typeof t.peerName === 'string' ? t.peerName.trim() : '') || '';
            const kind = (t && typeof t.kind === 'string' ? t.kind.trim() : '') || '';
            let subtitle = '';
            if (peer) {
                if (isIncome) {
                    subtitle = kind === 'redpacket' ? `来自 ${peer} 的红包` : `来自 ${peer}`;
                } else {
                    subtitle = kind === 'redpacket' ? `发给 ${peer} 的红包` : `发送给 ${peer}`;
                }
            }

            item.innerHTML = `
                <div class="wallet-item-left">
                    <div class="wallet-item-title">${t.title}</div>
