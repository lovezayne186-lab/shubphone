...
(function () {
  ...
  try {
    if (window.parent && window.parent !== window && window.parent.__musicOpenPlayerOnLoad) {
      ...
      scheduleWaveRebuild();
      syncFromGlobalPlayer();
      var pending = window.parent.__musicPendingOpenPayload && typeof window.parent.__musicPendingOpenPayload === "object"
        ? window.parent.__musicPendingOpenPayload
        : null;
      var togetherSession = pending && pending.togetherSession ? pending.togetherSession : null;
      try { window.parent.__musicPendingOpenPayload = null; } catch (e0) {}
