...
window.getWorldBookCheckboxListHTML = function (selectedIds) {
    ...
    const categories = [...new Set(books.map(b => b.category).filter(Boolean))];
    if (categories.length > 0) {
        html += `<div style="font-weight:bold; margin:10px 0 5px; font-size:12px; color:#666;">分类多选</div>`;
        categories.forEach(cat => {
            const id = `cat:${cat}`;
            const checked = selectedArr.includes(id) ? 'checked' : '';
            html += `
            <label style="display:flex; align-items:center; gap:8px; padding:6px 0; cursor:pointer;">
                <input type="checkbox" name="wb-checkbox" value="${id}" ${checked} style="width:16px; height:16px;">
                <span style="font-size:14px;">整个分类: ${cat}</span>
            </label>`;
        });
    }
    ...
    // 单本
    if (books.length > 0) {
        html += `<div style="font-weight:bold; margin:15px 0 5px; font-size:12px; color:#666;">单本多选</div>`;
        books.forEach(book => {
            const id = String(book.id || '');
            const checked = selectedArr.includes(id) ? 'checked' : '';
            html += `
            <label style="display:flex; align-items:center; gap:8px; padding:6px 0; cursor:pointer;">
                <input type="checkbox" name="wb-checkbox" value="${id}" ${checked} style="width:16px; height:16px;">
                <span style="font-size:14px;">${book.title || '未命名'}</span>
            </label>`;
        });
    }
    
    if (!html) html = '<div style="color:#999; font-size:13px; padding:10px 0;">暂无世界书，请先在世界书应用中添加。</div>';
